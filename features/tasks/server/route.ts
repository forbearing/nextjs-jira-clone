...
const app = new Hono()
  ...
  .post('/', sessionMiddleware, zValidator('json', createTaskSchema), async (c) => {
    ...
    const highestPositionTask = await databases.listDocuments(DATABASE_ID, TASK_ID, [
    ...
    const task = await databases.createDocument(DATABASE_ID, TASK_ID, ID.unique(), {
      name,
      status,
      workspaceId,
      projectId,
  ...
  .delete('/:taskId', sessionMiddleware, async (c) => {
    ...
    const task = await databases.getDocument<Task>(DATABASE_ID, TASK_ID, taskId)
  ...
  .patch('/:taskId', sessionMiddleware, zValidator('json', createTaskSchema.partial()), async (c) => {
    ...
    const { taskId } = c.req.param()
    const existingTask = await databases.getDocument<Task>(DATABASE_ID, TASK_ID, taskId)
    const member = await getMember({ databases, workspaceId: existingTask.workspaceId, userId: user.$id })
    if (!member) {
      return c.json({ error: 'Unauthorized' }, 401)
    }

    const task = await databases.updateDocument(DATABASE_ID, TASK_ID, taskId, {
      name,
      status,
      projectId,
      dueDate,
  ...
  .post(
    ...
    async (c) => {
      ...
      const tasksToUpdate = await databases.listDocuments<Task>(DATABASE_ID, TASK_ID, [
        Query.contains(
          '$id',
      ...
      const updatedTasks = await Promise.all(
        tasks.map(async (task) => {
          ...
          return databases.updateDocument<Task>(DATABASE_ID, TASK_ID, $id, { status, position })
        }),
      )
