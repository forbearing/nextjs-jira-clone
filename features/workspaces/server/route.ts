import { DATABASE_ID, IMAGE_BUCKET_ID, MEMBER_ID, PROJECT_ID, TASK_ID, WORKSPACE_ID } from '@/config'
...
const app = new Hono()
  ...
  .get('/:workspaceId/analytics', sessionMiddleware, async (c) => {
    ...
    const { workspaceId } = c.req.param()
    const workspace = await databases.getDocument<Workspace>(DATABASE_ID, WORKSPACE_ID, workspaceId)
    const member = await getMember({ databases, workspaceId: workspace.$id, userId: user.$id })
    if (!member) {
      return c.json({ error: 'Unauthorized' }, 401)
    }
    ...
    const thisMonthTasks = await databases.listDocuments<Task>(DATABASE_ID, TASK_ID, [
      Query.equal('workspaceId', workspaceId),
      Query.greaterThanEqual('$createdAt', thisMonthStart.toISOString()),
      Query.lessThanEqual('$createdAt', thisMonthEnd.toISOString()),
    ])
    const lastMonthTasks = await databases.listDocuments<Task>(DATABASE_ID, TASK_ID, [
      Query.equal('workspaceId', workspaceId),
    ...
    const thisMonthAssignedTasks = await databases.listDocuments<Task>(DATABASE_ID, TASK_ID, [
      Query.equal('workspaceId', workspaceId),
      Query.equal('assigneeId', member.$id),
      Query.greaterThanEqual('$createdAt', thisMonthStart.toISOString()),
      Query.lessThanEqual('$createdAt', thisMonthEnd.toISOString()),
    ])
    const lastMonthAssignedTasks = await databases.listDocuments<Task>(DATABASE_ID, TASK_ID, [
      Query.equal('workspaceId', workspaceId),
      Query.equal('assigneeId', member.$id),
      Query.greaterThanEqual('$createdAt', lastMonthStart.toISOString()),
      Query.lessThanEqual('$createdAt', lastMonthEnd.toISOString()),
    ...
    const thisMonthIncompleteTasks = await databases.listDocuments<Task>(DATABASE_ID, TASK_ID, [
      Query.equal('workspaceId', workspaceId),
      Query.notEqual('status', TaskStatus.DONE),
      Query.greaterThanEqual('$createdAt', thisMonthStart.toISOString()),
      Query.lessThanEqual('$createdAt', thisMonthEnd.toISOString()),
    ])
    const lastMonthIncompleteTasks = await databases.listDocuments<Task>(DATABASE_ID, TASK_ID, [
      Query.equal('workspaceId', workspaceId),
      Query.notEqual('status', TaskStatus.DONE),
      Query.greaterThanEqual('$createdAt', lastMonthStart.toISOString()),
      Query.lessThanEqual('$createdAt', lastMonthEnd.toISOString()),
    ...
    const thisMonthCompleteTasks = await databases.listDocuments<Task>(DATABASE_ID, TASK_ID, [
      Query.equal('workspaceId', workspaceId),
      Query.equal('status', TaskStatus.DONE),
      Query.greaterThanEqual('$createdAt', thisMonthStart.toISOString()),
      Query.lessThanEqual('$createdAt', thisMonthEnd.toISOString()),
    ])
    const lastMonthCompleteTasks = await databases.listDocuments<Task>(DATABASE_ID, TASK_ID, [
      Query.equal('workspaceId', workspaceId),
      Query.equal('status', TaskStatus.DONE),
      Query.greaterThanEqual('$createdAt', lastMonthStart.toISOString()),
      Query.lessThanEqual('$createdAt', lastMonthEnd.toISOString()),
    ...
    const thisMonthOverdueTasks = await databases.listDocuments<Task>(DATABASE_ID, TASK_ID, [
      Query.equal('workspaceId', workspaceId),
      Query.notEqual('status', TaskStatus.DONE),
    ...
    const lastMonthOverdueTasks = await databases.listDocuments<Task>(DATABASE_ID, TASK_ID, [
      Query.equal('workspaceId', workspaceId),
      Query.notEqual('status', TaskStatus.DONE),
      Query.lessThan('dueDate', now.toISOString()),
      Query.greaterThanEqual('$createdAt', lastMonthStart.toISOString()),
